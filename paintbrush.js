...
var paintBrush = {
    ...
    processFilters: function() {
        ...
        if (supports_canvas()) {
            ...
            paintBrush.addFilter("filter-blur", buffer, c);
            paintBrush.addFilter("filter-edges", buffer, c);
            paintBrush.addFilter("filter-emboss", buffer, c);
            paintBrush.addFilter("filter-greyscale", buffer, c);
            paintBrush.addFilter("filter-matrix", buffer, c);
            paintBrush.addFilter("filter-mosaic", buffer, c);
            paintBrush.addFilter("filter-noise", buffer, c);
            paintBrush.addFilter("filter-posterize", buffer, c);
            paintBrush.addFilter("filter-sepia", buffer, c);
            paintBrush.addFilter("filter-sharpen", buffer, c);
            paintBrush.addFilter("filter-tint", buffer, c);
        }
    },
    // the main workhorse function
    addFilter: function(filterType, buffer, c) {
        ...
        for(var current in toFilter) {
            ...
            var params = paintBrush.getFilterParameters(toFilter[current]);

            // get the image we're going to work with
            var img = paintBrush.getReferenceImage(toFilter[current]);
        ...
        function processFilters(filterType, img, params, toFilter, current, buffer, c) {
            ...
            if (img && c) {
                ...
                var pixels = paintBrush.initializeBuffer(c, img);
                ...
                if (pixels) {
                    ...
                    if (filterType == "filter-tint") {
                        var src  = parseInt(paintBrush.createColor(params.tintColor), 16),
                    ...
                    paintBrush.stashOriginal(img, originalSuffix, ref, buffer);
        ...
        function applyFilters(filterType, params, img, pixels, index, thisPixel, dest) {
            ...
            switch(filterType) {
                ...
                case "filter-greyscale":
                    ...
                    data = paintBrush.setRGB(data, index,
                        paintBrush.findColorDifference(params.greyscaleOpacity, val, thisPixel.r),
                        paintBrush.findColorDifference(params.greyscaleOpacity, val, thisPixel.g),
                        paintBrush.findColorDifference(params.greyscaleOpacity, val, thisPixel.b));
                    break;
                ...
                case "filter-mosaic":
                    ...
                    data = paintBrush.setRGB(data, index,
                        paintBrush.findColorDifference(params.mosaicOpacity, data[pos], thisPixel.r),
                        paintBrush.findColorDifference(params.mosaicOpacity, data[pos + 1], thisPixel.g),
                        paintBrush.findColorDifference(params.mosaicOpacity, data[pos + 2], thisPixel.b));
                    break;
                ...
                case "filter-posterize":
                    data = paintBrush.setRGB(data, index,
                        paintBrush.findColorDifference(params.posterizeOpacity, parseInt(params.posterizeValues * parseInt(thisPixel.r / params.posterizeAreas)), thisPixel.r),
                        paintBrush.findColorDifference(params.posterizeOpacity, parseInt(params.posterizeValues * parseInt(thisPixel.g / params.posterizeAreas)), thisPixel.g),
                        paintBrush.findColorDifference(params.posterizeOpacity, parseInt(params.posterizeValues * parseInt(thisPixel.b / params.posterizeAreas)), thisPixel.b));
                    break;

                case "filter-sepia":
                    data = paintBrush.setRGB(data, index,
                        paintBrush.findColorDifference(params.sepiaOpacity, (thisPixel.r * 0.393) + (thisPixel.g * 0.769) + (thisPixel.b * 0.189), thisPixel.r),
                        paintBrush.findColorDifference(params.sepiaOpacity, (thisPixel.r * 0.349) + (thisPixel.g * 0.686) + (thisPixel.b * 0.168), thisPixel.g),
                        paintBrush.findColorDifference(params.sepiaOpacity, (thisPixel.r * 0.272) + (thisPixel.g * 0.534) + (thisPixel.b * 0.131), thisPixel.b));
                    break;

                case "filter-tint":
                    data = paintBrush.setRGB(data, index,
                        paintBrush.findColorDifference(params.tintOpacity, dest.r, thisPixel.r),
                        paintBrush.findColorDifference(params.tintOpacity, dest.g, thisPixel.g),
                        paintBrush.findColorDifference(params.tintOpacity, dest.b, thisPixel.b));
                    break;
        ...
        function applyMatrix(img, pixels, matrix, amount) {
            ...
            for (var i = 1; i < imgWidth - 1; i++) {
                for (var j = 1; j < img.height - 1; j++) {
                    ...
                    data = paintBrush.setRGB(data, ref,
                        paintBrush.findColorDifference(amount, sumR, thisPixel.r),
                        paintBrush.findColorDifference(amount, sumG, thisPixel.g),
                        paintBrush.findColorDifference(amount, sumB, thisPixel.b));
                }
            }
    ...
    getFilterParameters: function(ref) {
    ...
    initializeBuffer: function(c, img) {
    ...
    setRGB: function(data, index, r, g, b) {
        data[index] = r;
        data[index + 1] = g;
        data[index + 2] = b;
        return data;
    ...
    getReferenceImage: function(ref) {
        if (ref.nodeName == "IMG") {
    ...
    placeReferenceImage: function(ref, result, img) {
    ...
    flushDataAttributes: function(img) {
    ...
    destroyStash: function(img, preserve) {
    ...
    stashOriginal: function(img, originalSuffix, ref, buffer) {
        // store the original image in the DOM
        var stashed = paintBrush.stashInDom(img, originalSuffix);

        // then replace the original image data with the buffer
        paintBrush.placeReferenceImage(ref, buffer.toDataURL("image/png"), img);
    ...
    stashInDom: function(img, originalSuffix) {
